...
import { Box, Calendar, Truck, DollarSign, LogOut, Plus, Check, X as XIcon, Trash2, Edit2, LayoutDashboard, Shovel, AlertCircle } from "lucide-react";
...
export default function PortailOperations() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'interne' | 'externe'>('interne');
  ...
  const [editCamionForm, setEditCamionForm] = useState({ numero: "", chauffeur: "", type: "interne" });
  ...
  useEffect(() => {
    ...
    const unsubM = onSnapshot(collection(db, "maintenances"), (snap) => {
  ...
  const groupedPartenaireData = useMemo(() => {
    const externeChargements = filteredData.filter(c => c.typeTransporteur === 'externe');
    ...
    return Object.values(groups).sort((a, b) => new Date(b.dossier?.createdAt || 0).getTime() - new Date(a.dossier?.createdAt || 0).getTime());
  }, [filteredData]);

  // --- Flotte Logic ---
  const groupedFlotteData = useMemo(() => {
    const interneChargements = filteredData.filter(c => c.typeTransporteur === 'interne');
    const groups: { [key: string]: { dossier: any, items: any[] } } = {};
  ...
  const handleAssignCamion = async (camionId: string) => {
    ...
    } catch (err) { handleFirestoreError(err, OperationType.UPDATE, `chargements/${assigningMission.id}`); }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans transition-colors duration-300">
      ...
      <header className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-30 shadow-xl">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-base font-bold uppercase text-white tracking-tight">Portail <span className="text-blue-500">Opérations</span></h1>
              <p className="text-[9px] font-medium uppercase tracking-widest text-slate-500">Gestion Logistique</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 pb-24">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-center gap-4">
           <div className="flex-1 w-full space-y-1">
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest px-1">Filtre Date</p>
           ...
           <div className="flex-1 w-full space-y-1">
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest px-1">Référence BL / Conteneur</p>
              <input 
           ...
           <button 
             onClick={() => { setDateRange({start: "", end: ""}); setBlSearch(""); }}
        ...
        {activeTab === 'flotte' ? (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">Unités de Flotte</h2>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Flotte {activeSubTab === 'interne' ? 'en CDI' : 'Partenaire'}</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                 ...
                 <button 
                   onClick={() => { setShowNewCamion(true); setNewCamion({...newCamion, type: activeSubTab}); }}
            ...
            {showNewCamion && (
              <form onSubmit={handleCreateCamion} className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <input 
                ...
                <div className="flex gap-2">
                  <button type="submit" className="flex-1 bg-emerald-600 text-white font-bold uppercase text-[9px] rounded-lg">Enregistrer</button>
                  <button onClick={() => setShowNewCamion(false)} className="px-3 text-slate-400 font-bold uppercase text-[9px]">Annuler</button>
                </div>
              </form>
            )}
            ...
            <div className="space-y-4">
               { (activeSubTab === 'interne' ? groupedFlotteData : groupedExterneData).map(group => (
                 <div key={group.dossier?.id || 'unassigned'} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                    <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                       <span className="text-[10px] font-bold text-slate-900 dark:text-white uppercase tracking-tight">#{group.dossier?.numeroBL || 'SANS DOSSIER'} • <span className="text-slate-500">{group.dossier?.client || 'Client Inconnu'}</span></span>
                       <span className="text-[10px] font-medium text-slate-400">{group.items.length} unité(s)</span>
                    </div>
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                       {group.items.map(miss => {
                         const camion = camions.find(c => c.id === miss.camionId);
                         return (
                           <div key={miss.id} onClick={() => setAssigningMission(miss)} className="px-4 py-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer transition-colors">
                              <div className="flex items-center gap-4">
                                <span className="text-xs font-bold text-slate-900 dark:text-white">{miss.numeroConteneur}</span>
                                <span className="text-[9px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded uppercase text-slate-500 font-bold">{miss.ville}</span>
                              </div>
                              <div className="flex items-center gap-3">
                                {camion ? (
                                  <div className="flex items-center gap-2">
                                    <Truck className="w-3 h-3 text-blue-500" />
                                    <span className="text-[10px] font-bold uppercase text-slate-700 dark:text-slate-300">{camion.numero}</span>
                                  </div>
                                ) : (
                                  <span className="text-[9px] text-rose-500 font-bold uppercase animate-pulse">À affecter</span>
                                )}
                              </div>
                           </div>
                         );
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {camions.filter(c => activeSubTab === 'interne' ? c.type !== 'externe' : c.type === 'externe').map(c => (
                    <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 text-[11px] transition-colors">
                      <td className="px-4 py-3">
                        {isEditingCamion === c.id ? (
                          <input 
                            ...
                            onKeyDown={e => e.key === 'Enter' && handleEditCamion(e as any, c.id)}
                            autoFocus
                          />
                        ) : (
                          <span className="font-bold text-slate-900 dark:text-white uppercase">{c.numero}</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        {isEditingCamion === c.id ? (
                          <input 
                            ...
                            onChange={e => setEditCamionForm({...editCamionForm, chauffeur: e.target.value})}
                            onKeyDown={e => e.key === 'Enter' && handleEditCamion(e as any, c.id)}
                          />
                        ) : (
                          <span className="text-slate-500 uppercase font-medium">{c.chauffeur || 'N/A'}</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <select 
                          value={c.statut}
                          onChange={(e) => toggleCamionStatut(c.id, e.target.value)}
                          className={`text-[9px] font-bold uppercase px-2 py-1 rounded bg-transparent outline-none cursor-pointer ${
                            c.statut === 'actif' ? 'text-emerald-600' : 
                            c.statut === 'panne' ? 'text-rose-600' : 'text-amber-600'
                          }`}
                        >
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2 text-slate-400">
                          {isEditingCamion === c.id ? (
                            <button onClick={(e) => handleEditCamion(e as any, c.id)} className="text-emerald-500 hover:scale-110 transition-transform"><Check className="w-4 h-4" /></button>
                          ) : (
                            <button onClick={() => { setIsEditingCamion(c.id); setEditCamionForm({ numero: c.numero, chauffeur: c.chauffeur, type: c.type || activeSubTab }); }} className="hover:text-blue-500 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                          )}
                          <button onClick={() => setDeleteCamionId(c.id)} className="hover:text-rose-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Missions Externes</p>
                <p className="text-xl font-bold text-slate-900 dark:text-white">{groupedPartenaireData.reduce((sum, g) => sum + g.items.length, 0)}</p>
              </div>
              <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Acomptes</p>
                <p className="text-xl font-bold text-emerald-600">{(chargements.filter(c => c.typeTransporteur === 'externe').reduce((sum, c) => sum + (Number(c.avance) || 0), 0)).toLocaleString()}</p>
              </div>
              <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Solde à Payer</p>
                <p className="text-xl font-bold text-amber-600">{(chargements.filter(c => c.typeTransporteur === 'externe').reduce((sum, c) => sum + (Number(c.prixTotal) || 0), 0) - chargements.filter(c => c.typeTransporteur === 'externe').reduce((sum, c) => sum + (Number(c.avance) || 0), 0)).toLocaleString()}</p>
              </div>
            </div>
               ...
               {groupedPartenaireData.map(group => (
                 <div key={group.dossier?.id || 'unassigned'} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                    <div className="px-4 py-2 bg-amber-50 dark:bg-amber-900/10 border-b border-amber-100 dark:border-amber-900/20 flex justify-between items-center">
                       <h4 className="text-[10px] font-bold text-slate-900 dark:text-white uppercase tracking-tight">Dossier BL: #{group.dossier?.numeroBL || 'SANS DOSSIER'}</h4>
                       <span className="text-xs font-bold text-amber-600">{(group.items.reduce((sum, item) => sum + (Number(item.prixTotal) || 0), 0)).toLocaleString()} {settings.devise}</span>
                    </div>
                    <table className="w-full text-left border-collapse">
                       <thead>
                       ...
                       <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                         {group.items.map(miss => {
                           const camion = camions.find(c => c.id === miss.camionId);
                           return (
                             <tr key={miss.id} onClick={() => setAssigningMission(miss)} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer text-[11px] transition-colors">
                               <td className="px-4 py-3 font-bold text-slate-900 dark:text-white uppercase">{miss.numeroConteneur}</td>
                               <td className="px-4 py-3 text-slate-500 font-medium uppercase">{miss.ville}</td>
                               <td className="px-4 py-3 font-bold text-slate-700 dark:text-slate-300 uppercase">{camion ? camion.numero : 'Non affecté'}</td>
                               <td className="px-4 py-3 text-right">
                                 <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded ${camion ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700 animate-pulse'}`}>
                                   {camion ? 'Assigné' : 'Attente'}
                                 </span>
                               </td>
                             </tr>
                           );
      ...
      {assigningMission && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" onClick={() => setAssigningMission(null)} />
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
               <div>
                 <p className="text-[9px] font-bold text-blue-500 uppercase tracking-widest mb-1">Assignation Unité</p>
               ...
               <button onClick={() => setAssigningMission(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
                 <XIcon className="w-5 h-5 text-slate-400" />
               </button>
            </div>
            
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
               <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Sélectionner un transporteur ({assigningMission.typeTransporteur}) :</p>
               <div className="grid grid-cols-1 gap-2">
                 {camions
                   .filter(c => c.statut === 'actif' && c.type === (assigningMission.typeTransporteur === 'interne' ? 'interne' : 'externe'))
                   .map(c => (
                     <button
                       key={c.id}
                     ...
                     >
                       <div className="flex items-center gap-3">
                         <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${c.type === 'interne' ? 'bg-blue-600' : 'bg-amber-500'} text-white`}>
                           <Truck className="w-4 h-4" />
                         </div>
                         <div>
                           <p className="font-bold text-slate-900 dark:text-white uppercase text-sm leading-none">{c.numero}</p>
                           <p className="text-[9px] font-medium text-slate-400 uppercase tracking-wider mt-1">{c.chauffeur || "Sans chauffeur"}</p>
                         </div>
                       </div>
                 ...
                 {camions.filter(c => c.statut === 'actif' && c.type === (assigningMission.typeTransporteur === 'interne' ? 'interne' : 'externe')).length === 0 && (
                   <div className="text-center py-6 bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-dotted border-slate-200 dark:border-slate-800">
                     <p className="text-slate-400 font-bold uppercase text-[9px] tracking-widest">Aucune unité active disponible</p>
                   </div>
                 )}
               </div>
            </div>
...
function SummaryCardPartenaire({ label, value, icon: Icon, color, highlight = false }: any) {
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4 ${highlight ? 'ring-1 ring-amber-500/30' : ''}`}>
       <div className={`p-2.5 rounded-lg ${colors[color] || colors.blue} shrink-0`}>
         <Icon className="w-5 h-5" />
       </div>
       <div>
         <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1">{label}</p>
         <h3 className="text-base font-bold text-slate-900 dark:text-white tabular-nums tracking-tight">{value}</h3>
       </div>
    </div>
  );
}
...
function StatRow({ label, value, sub, highlight = false }: any) {
  return (
    <div className="p-4 sm:border-r sm:last:border-r-0 border-b sm:border-b-0 last:border-b-0 border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
       <p className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">{label}</p>
       <p className={`text-base font-bold tabular-nums ${highlight ? 'text-blue-600' : 'text-slate-900 dark:text-white'}`}>{value}</p>
       <p className="text-[9px] font-medium text-slate-400 dark:text-slate-500 uppercase tracking-normal mt-1">{sub}</p>
    </div>
  );
}
